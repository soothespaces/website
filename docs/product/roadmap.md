...
- [ ] Harden the validated MPrint room-extraction prototype ([writeup](../technical/mprint-extraction.md)):
      auto-calibrate dilation radius, validate OCR against `rooms.json`, run across
      more buildings before trusting the output
- [ ] Build the internal floor-plan alignment tool
      ([ADR 0005](../decisions/0005-manual-floor-plan-alignment-tool.md)): drag/
      rotate/scale a footprint ghost outline against each MPrint image, save the
      resulting 4 corner coordinates to that floor's `FloorPlan` record
...
- [ ] Live occupancy card — blocked on our own Waitz API access (Phase 1) and on
      ...
      yet
- [ ] MPrint floor-plan viewer, rendered as a geographically-anchored map overlay
      (decided — see [Architecture](../technical/architecture.md)) using the corner
      coordinates from the Phase 1 alignment tool, with clickable per-zone reviews
      once the extraction pipeline is hardened (see
...
- [ ] 2.5D building massing via `fill-extrusion` (`floors × ~3.5m`, no new data
      needed — see [Architecture](../technical/architecture.md))
